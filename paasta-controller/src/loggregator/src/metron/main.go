...
func main() {
	...
	fmt.Println("##### metron/main.go - logFilePath :", *logFilePath)
	fmt.Println("##### metron/main.go - configFilePath :", *configFilePath)
	fmt.Println("##### metron/main.go - config :", config)
...
func initializeDopplerPool(conf *config.Config, batcher *metricbatcher.MetricBatcher, logger *gosteno.Logger) (*eventmarshaller.EventMarshaller, error) {
	...
	for protocol := range conf.Protocols {
		...
		fmt.Println("##### initializeDopplerPool : proto :", proto)
